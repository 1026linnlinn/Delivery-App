...
//        detailsAdapter = new DetailsAdapter(this);
        ...
        apiInterface.getServiceDetail().enqueue(new Callback<GetServiceDetails>() {
            @Override
            public void onResponse(Call<GetServiceDetails> call, Response<GetServiceDetails> response) {
                if (response.isSuccessful()) {
                    categoryName.setText(response.body().getServiceName);
//                   adapter.addData(response.body().getServiceProducts);
                } else {
